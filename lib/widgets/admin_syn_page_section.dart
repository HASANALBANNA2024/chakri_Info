...
class _AdminSyncPageState extends State<AdminSyncPage> {
  ...
  final List<String> _step1Main = ['All', 'Job Circular', 'Question Bank', 'Admission', 'Notice or Result'];
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                if (docs.isEmpty) return const Center(child: Text("কোনো ডাটা পাওয়া যায়নি।"));
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(
            children: [
              Expanded(child: _buildSyncDropdown("ধাপ ১", selectedStep1, _step1Main, (val) => setState(() { selectedStep1 = val!; selectedStep2 = selectedStep3 = selectedStep4 = null; }))),
              if (selectedStep1 != "All" && _step2Sub.containsKey(selectedStep1)) ...[
                const SizedBox(width: 8),
                Expanded(child: _buildSyncDropdown("ধাপ ২", selectedStep2, _step2Sub[selectedStep1]!, (val) => setState(() { selectedStep2 = val == "All" ? null : val; selectedStep3 = selectedStep4 = null; }))),
              ],
            ],
          ),
          if (selectedStep2 != null && _step3Specific.containsKey(selectedStep2)) ...[
            ...
            Row(
              children: [
                Expanded(child: _buildSyncDropdown("ধাপ ৩", selectedStep3, _step3Specific[selectedStep2]!, (val) => setState(() { selectedStep3 = val == "All" ? null : val; selectedStep4 = null; }))),
                if (selectedStep3 != null && _step4Final.containsKey(selectedStep3)) ...[
                  const SizedBox(width: 8),
                  Expanded(child: _buildSyncDropdown("ধাপ ৪", selectedStep4, _step4Final[selectedStep3]!, (val) => setState(() => selectedStep4 = val == "All" ? null : val))),
                ],
              ],
            ),
          ],
  ...
  Widget _buildSyncDropdown(String label, String? value, List<String> items, Function(String?) onChanged) {
    return DropdownButtonFormField<String>(
      value: value,
      isExpanded: true,
      decoration: InputDecoration(labelText: label, contentPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 0), border: OutlineInputBorder(borderRadius: BorderRadius.circular(8))),
  ...
  Widget _buildJobCard(DocumentSnapshot doc, Map<String, dynamic> data) {
    return Card(
      ...
      child: ListTile(
        leading: ClipRRect(borderRadius: BorderRadius.circular(8), child: _buildLeadingImage(data['images'])),
        ...
        trailing: PopupMenuButton(
          onSelected: (val) { if (val == 'edit') _showEditSheet(doc, data); if (val == 'delete') _jobRepo.deleteJob(doc.reference); },
  ...
  Widget _buildLeadingImage(dynamic images) {
    if (images != null && images is List && images.isNotEmpty) {
      try { return Image.memory(base64Decode(images[0]), width: 50, height: 50, fit: BoxFit.cover); } catch (e) { return const Icon(Icons.image); }
    }
    ...
    showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setSheetState) => Padding(
          padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom), // কিবোর্ডের জন্য প্যাডিং
          child: Container(
            constraints: BoxConstraints(
              maxHeight: MediaQuery.of(context).size.height * 0.9, // হাইট স্ক্রিনের ৯০% এর বেশি হবে না
            ),
                          ...
                          onPressed: () async {
                            ...
                            if (pickedFiles.isNotEmpty) {
                              for (var file in pickedFiles) {
                                ...
                                setSheetState(() { currentImages.add(base64Encode(imageBytes)); });
                              }
                            }
                          },
                          icon: const Icon(Icons.add_a_photo, size: 18),
                          label: const Text("নতুন ছবি", style: TextStyle(fontSize: 12))
                      ),
                    ],
                  ),
                      ...
                      itemBuilder: (context, i) => Stack(
                        children: [
                          Container(
                            ...
                            width: 100,
                            decoration: BoxDecoration(borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.grey.shade300), image: DecorationImage(image: MemoryImage(base64Decode(currentImages[i])), fit: BoxFit.cover)),
                          ),
                          Positioned(top: 0, right: 5, child: GestureDetector(onTap: () => setSheetState(() => currentImages.removeAt(i)), child: const CircleAvatar(radius: 12, backgroundColor: Colors.red, child: Icon(Icons.close, size: 14, color: Colors.white)))),
                        ],
                      ),
                    ),
                  ),
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () async {
                        await doc.reference.update({
                          ...
                          'positions': posControllers.map((p) => {'name': p['name']!.text, 'post': p['post']!.text, 'salary_grade': p['salary']!.text}).toList(),
                        });
                        Navigator.pop(context);
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("সবকিছু সফলভাবে আপডেট হয়েছে!")));
                      },
                      child: const Text("Update Everything", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                    ),
                  ),
  ...
  Widget _editField(TextEditingController ctrl, String label, {int maxLines = 1}) {
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(labelText: label, border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)), contentPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8)),
      ),
    );
  }
}